...
st.title("Gerenciamento de Pallets Mariza Group V1.0")
...
exportar_excel = st.button("Exportar para Excel")
if exportar_excel:
    # Criando um buffer de memória para o arquivo Excel
    output = BytesIO()
    
    # Salvando o DataFrame no buffer como Excel
    with pd.ExcelWriter(output, engine="xlsxwriter") as writer:
        df_filtrado.to_excel(writer, index=False, sheet_name="Relatorio")
    
    # Configurando o download do arquivo Excel
    st.download_button(
        label="Baixar Excel",
        data=output.getvalue(),
        file_name="relatorio_pallets.xlsx",
